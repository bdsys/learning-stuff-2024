...
exports.genre_delete_post = function(req, res, next) {
    ...
    console.error("DEBUG: request body genreid parameter: " + req.body.genreid);

    async.parallel({
        genre: function(callback) {
          Genre.findById(req.body.genreid).exec(callback)
        },
        genre_books: function(callback) {1
        ...
        else if (results.genre == undefined) {
            // No genre found. Render genre_delete with error
            console.error("DEBUG: No genre found triggered!");
            let custom_errors = ["Genre not found with Id: " + req.body.genreid];
            res.render('genre_delete', { title: 'Delete Genre', genre: results.genre, genre_books: results.genre_books, custom_errors: custom_errors } );
            return;
        }
        else {
